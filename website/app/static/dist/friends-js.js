...
function sendPartyInvite(userId){reqSendPartyInvite(userId);}
...
const result=await res.json();console.log(result);}
async function reqAcceptPartyInvite(userTag){let formData=new FormData();formData.append('user_tag',userTag);const res=await fetch('/friends/accept_party_invite',{method:"POST",body:formData})
const result=await res.json();console.log(result);}
